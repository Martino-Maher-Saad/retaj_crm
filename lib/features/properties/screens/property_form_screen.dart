  ...
  Widget build(BuildContext context) {
    return ListenableBuilder(
      ...
      builder: (context, state) {
        ...
        return Container(
          width: screenWidth.clamp(360.0, 900.0),
                            ...
                            children: [
                              Expanded(
                                child: _buildDropdown(
                                  "نوع الإعلان",
                                  dataManager.getActiveOptions(
                                    'listing_type',
                                    includeValue: selectedListingType,
                                  ),
                                  selectedListingType,
                                  (v) =>
                                      setState(() => selectedListingType = v),
                                ),
                              ),
                              SizedBox(width: 14.w),
                              Expanded(
                                child: _buildDropdown(
                                  ...
                                  dataManager.getActiveOptions(
                                    'property_type',
                                    includeValue: selectedPropertyType,
                                  ),
                      ...
                      children: [
                          ...
                          Container(
                            ...
                            child: intl_phone_field.IntlPhoneField(
                              ...
                              initialValue: _controllers['ownerPhone']!.text,
                              onChanged: (phone) {
                                _controllers['ownerPhone']!.text =
                          ...
                          if (widget.userRole == 'admin' ||
                            ...
                            Wrap(
                              ...
                              children: dataManager
                                  .getActiveOptions('advertising_platforms')
                                  .map((platform) {
                                    final isSelected = selectedPlatforms
                                        .contains(platform);
                                    return FilterChip(
                                      label: Text(
                                        platform,
  ...
  Future<void> _submit() async {
    ...
    if (widget.property != null) {
      ...
      final existingSorted =
          p.advertisingPlatforms.map((e) => e.nameAr).toList()..sort();
      final bool platformsChanged =
          selectedSorted.join('|') != existingSorted.join('|');

      // مقارنة الصور
      final bool imagesChanged =
          _newImagesBytes.isNotEmpty || _imagesToDeleteObjects.isNotEmpty;
    ...
    // ─── Duplicate Check للمالك (إضافة جديدة فقط) ───
    if (widget.property == null) {
      final ownerPhone = _controllers['ownerPhone']!.text.trim();
      if (ownerPhone.isNotEmpty) {
        try {
          final duplicates = await Supabase.instance.client.rpc(
            'check_property_owner_phone_duplicate',
            params: {'p_phone': ownerPhone},
          );

          if (duplicates != null && (duplicates as List).isNotEmpty) {
            final dupTitle = duplicates.first['title_ar'] ?? 'عقار غير معروف';
            final dupCode = duplicates.first['property_code'] ?? '';
            ...
            final bool? confirm = await showDialog<bool>(
              ...
              builder: (ctx) => AlertDialog(
                ...
                content: Column(
                  ...
                  children: [
                    Text(
                      'يوجد عقار آخر مسجل بنفس رقم هاتف المالك:',
                    ...
                    SizedBox(height: 12.h),
                    Text(
                      'هل أنت متأكد أنك تريد إضافة هذا العقار على أي حال؟',
                      style: TextStyle(
                        fontWeight: FontWeight.bold,
                        fontSize: 14.sp,
                      ),
          ...
          // لو الـ RPC فشلت نستمر بشكل عادي
        }
      }
    }
    ...
    try {
      ...
      if (finalCode.isEmpty) {
        finalCode =
            "PROP-${DateTime.now().millisecondsSinceEpoch.toString().substring(5)}";
      }
      ...
      final model = PropertyModel(
        ...
        price:
            num.tryParse(_controllers['price']!.text.replaceAll(',', '')) ?? 0,
      ...
      if (widget.property == null) {
        ...
        final duplicates = await context
            .read<PropertiesCubit>()
            .checkDuplicates(phone);
        final myDuplicates = duplicates
            .where((d) => d.createdBy == widget.userId)
            .toList();
          ...
          final bool? confirm = await showDialog<bool>(
            ...
            builder: (ctx) => AlertDialog(
              ...
              content: Column(
                ...
                children: [
                  Text(
                    "يوجد عقار آخر مضاف بواسطتك يمتلك نفس رقم المالك (أو يتطابق في آخر 6 أرقام).",
                  ...
                  SizedBox(height: 15.h),
                  Text(
                    "هل أنت متأكد أنك تريد إضافة هذا العقار كعقار جديد على أي حال؟",
                    style: TextStyle(
                      fontSize: 16.sp,
                      fontWeight: FontWeight.bold,
                    ),
        ...
        if (mounted) {
          context.read<PropertiesCubit>().addProperty(
            model,
            _newImagesBytes,
      ...
      ScaffoldMessenger.of(
        context,
      ).showSnackBar(SnackBar(content: Text("خطأ في معالجة البيانات: $e")));
      setState(() => _isLoading = false);
    }
  }
