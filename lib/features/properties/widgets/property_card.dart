...
import 'platform_status_dialog.dart';
...
class PropertyCard extends StatefulWidget {
  ...
  /// لمنع تعديل أي بيانات غير حالة المنصات (للماركتينج في صفحة إدارة الإعلانات)
  final bool platformOnlyMode;

  /// callback بيتم استدعاءه بعد حفظ تعديلات المنصات عشان يتم تحديث القائمة في صفحة إدارة الإعلانات
  final ValueChanged<PropertyModel>? onPlatformSaved;
...
class _PropertyCardState extends State<PropertyCard> {
  ...
  void _initInlineEditData() {
    ...
    _selectedPlatforms = [
      ...widget.property.targetPlatforms,
    ...
    _propertyCodeController.addListener(_onFieldChanged);
  ...
  Future<void> _saveInlineProperty() async {
    ...
    if (_newImagesBytes.isEmpty && _existingImages.isEmpty) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text('صورة واحدة على الأقل مطلوبة')),
      );
      hasError = true;
    }
    String typedCode = _propertyCodeController.text.trim();
    ...
    if (finalCode != '0' &&
      ...
      final isCodeExist = await context
          ...
          .checkPropertyCodeExists(finalCode);
      if (isCodeExist) {
        _propertyCodeError = 'كود العقار مستخدم مسبقاً';
        hasError = true;
      }
    }
    ...
    try {
      ...
      final cityObj = dataManager.allCities.firstWhere(
        (c) => c.id == _selectedCityId,
        orElse: () => dataManager.allCities.first,
      );
      ...
      if (widget.isAddingMode) {
        // Collect platform IDs for adding
        ...
        final bool updateEmbeddings = _descController.text.trim() != (widget.property.descAr ?? '');
        ...
        if (model.ownerName != widget.property.ownerName) {
          partialUpdates['owner_name'] = model.ownerName;
        }
        if (model.ownerPhone != widget.property.ownerPhone) {
          partialUpdates['owner_phone'] = model.ownerPhone;
        }
        if (model.descAr != widget.property.descAr) {
          partialUpdates['desc_ar'] = model.descAr;
        }
        ...
        await context.read<PropertiesCubit>().updateProperty(
          ...
          platformIds: pIds,
          partialUpdates: partialUpdates.isEmpty ? null : partialUpdates,
                    ...
                    child: Align(
                      ...
                      child: Column(
                        ...
                        children: [
                          ...
                          if (widget.property.waitingPlatforms.isNotEmpty ||
                              widget.property.targetPlatforms.isNotEmpty ||
                              widget.property.suspendedPlatforms.isNotEmpty) ...[
                            SizedBox(height: 8.h),
                            Builder(
                              builder: (context) {
                                final allBadges = [
                                ...
                                if (allBadges.length > 2) {
                                  displayBadges = allBadges.take(2).toList();
                                  displayBadges.add(
                                    GestureDetector(
                                      onTap: _openPlatformStatusDialog,
                                      child: _buildMiniBadge('+${allBadges.length - 2}', Colors.grey.shade700),
                                    ),
                                  );
                                }
                            ...
                            children: [
                              Expanded(
                                ...
                                child: _buildDetailRow(
                                  "أضيف بواسطة",
                                  widget.property.createdByName ?? "غير معروف",
                                  icon: Icons.person,
                                ),
                              ),
                              Expanded(
                                ...
                                child: _buildDetailRow(
                                  "كود العقار",
                                  widget.property.propertyCode ?? "غير متوفر",
                                ),
                              ),
                            ],
                          ),
                          ...
                          Row(
                            ...
                            children: [
                              Expanded(
                                child: _buildDetailRow(
                                  "نوع العقار",
                                  widget.property.propertyTypeAr,
                                ),
                              ),
                              Expanded(
                                child: _buildDetailRow(
                                  "نوع الإعلان",
                                  widget.property.listingTypeAr,
                                ),
                              ),
                              Expanded(
                                child: _buildDetailRow(
                                  "المدينة",
                                  widget.property.cityAr ?? "غير محدد",
                                ),
                              ),
                            ],
                          ),
                            ...
                            children: [
                              if (!shouldMask) ...[
                                Expanded(
                                  ...
                                  child: _buildDetailRow(
                                    "اسم المالك",
                                    widget.property.ownerName ?? "غير متوفر",
                                  ),
                                ),
                                Expanded(
                                  flex: 2,
                              ...
                              if (widget.property.managerNotes?.isNotEmpty ==
                                      true ||
                                  (widget.property.approvalStatusName?.contains(
                                            'موافق',
                                          ) ==
                                          true ||
                                      widget.property.approvalStatusName
  ...
  Widget _buildEditMode() {
    final dataManager = di.sl<StaticDataManager>();
    final listingOptions = dataManager
        .getOptions('listing_type')
        .toSet()
        .toList();
    final propOptions = dataManager
                  ...
                  children: [
                      ...
                      children: [
                        ...
                        for (int i = 0; i < _newImagesBytes.length; i++)
                          _buildMemoryThumbnail(
                            _newImagesBytes[i],
                            () => setState(() => _newImagesBytes.removeAt(i)),
                          ),
                        if ((_existingImages.length + _newImagesBytes.length) <
                            10)
                          GestureDetector(
                            onTap: _pickImages,
                            child: Container(
                    ...
                    if (widget.role == 'admin' ||
                      ...
                      Wrap(
                        ...
                        children: [
                          ...(() {
                            final platformsOptions = [...dataManager.getActiveOptions('advertising_platform')];
                            if (!platformsOptions.contains('بدون إعلان')) {
                              platformsOptions.insert(0, 'بدون إعلان');
                            }
                            return platformsOptions.map((platform) {
                              ...
                              return SizedBox(
                                ...
                                child: FilterChip(
                                  ...
                                  selectedColor: platform == 'بدون إعلان'
                                      ? Colors.red
                                      : AppColors.brandPrimary,
            ...
            Column(
              ...
              children: [
                ElevatedButton.icon(
                  ...
                  label: Text(
                    widget.isAddingMode ? "حفظ" : "حفظ التعديل",
                    style: TextStyle(
                      fontWeight: FontWeight.bold,
                      fontSize: 16.sp,
                    ),
                  ...
                  onPressed: (_isSavingInline || !_hasChanges) ? null : _saveInlineProperty,
                ),
                SizedBox(height: 12.h),
                OutlinedButton.icon(
  ...
  Color _getApprovalColor(String? status) {
    ...
    if (status.contains('موافق') || status.contains('مقبول') || status.contains('تم النشر')) {
      return Colors.green;
    }
    if (status.contains('مرفوض')) return Colors.red;
    if (status.contains('مسودة')) return Colors.grey.shade700;
    return Colors.orange; // 'قيد المراجعة' or other
  }
  ...
  void _openPlatformStatusDialog() {
    final authState = context.read<AuthCubit>().state;
    if (authState is AuthSuccess) {
      final user = authState.user;
      if (user.canManagePlatforms) {
        // لو الكارت جوه صفحة إدارة الإعلانات، نستخدم onPlatformSaved callback
        // عشان يتحدث قائمة MarketingCubit بدل PropertiesCubit
        if (widget.onPlatformSaved != null) {
          showDialog(
            context: context,
            builder: (dialogContext) => PlatformStatusDialog(
              ...
              onSaved: (updatedProperty) {
                ...
                // تحديث القائمة في صفحة إدارة الإعلانات
                widget.onPlatformSaved!(updatedProperty);
              },
            ),
          );
        } else {
        ...
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(content: Text('ليس لديك صلاحية لإدارة المنصات')),
        );
      }
    }
  }
