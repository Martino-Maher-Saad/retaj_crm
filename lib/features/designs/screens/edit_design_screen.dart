...
class _EditDesignScreenState extends State<EditDesignScreen> {
  ...
  final List<dynamic> _oldImages = [];
  final List<String> _imagesToDeleteIds = [];
  ...
  void initState() {
    super.initState();
    _descController.text = widget.design.descAr ?? '';
    _selectedRoomType = widget.design.roomType;
    _selectedStyle = widget.design.style;
    _oldImages.addAll(widget.design.images ?? []);
  }
  ...
  void _submit() async {
    ...
    final updatedFields = {
      'desc_ar': _descController.text,
      'room_type': _selectedRoomType,
      'style': _selectedStyle,
    };

    try {
      await context.read<DesignsCubit>().updateDesign(
        designId: widget.design.id,
        updatedFields: updatedFields,
        newImagesBytes: _newImagesBytes,
        imagesToDeleteIds: _imagesToDeleteIds,
      );
      if (mounted) {
                      ...
                      Column(
                        ...
                        children: [
                          ...
                          if (_oldImages.isNotEmpty) ...[
                            SizedBox(height: 16.h),
                            Text("الصور الحالية:", style: AppTextStyles.inputText.copyWith(fontWeight: FontWeight.bold)),
                            SizedBox(height: 8.h),
                            Wrap(
                              spacing: 8.w,
                              runSpacing: 8.h,
