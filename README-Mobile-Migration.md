# دليل هجرة واجهة المستخدم للهواتف المحمولة (Mobile UI Migration Guide) - Retaj CRM

هذا الملف مخصص للنموذج المبرمج الذي سيتولى تحويل واجهة المستخدم الخاصة بتطبيق Retaj CRM من واجهة سطح مكتب (Desktop/Web) إلى واجهة متوافقة مع الهواتف المحمولة (Mobile App)، مع الحفاظ على **المنطق (Logic)** و **تدفق البيانات (Data Flow)** الحاليين دون أي تخريب.

---

## 1. نظرة عامة على المشروع (Project Overview)
- **طبيعة التطبيق:** نظام إدارة علاقات العملاء (CRM) مخصص لشركة عقارية (Retaj).
- **إطار العمل:** Flutter
- **تخطيط الواجهة الحالي:** سطح مكتب/ويب باستخدام `flutter_screenutil` ومضبوط على دقة `(1920x1080)`.
- **الباك إند وقاعدة البيانات:** Supabase.

---

## 2. المنطق المعماري وإدارة الحالة (Logic & State Management)
منعاً لإحداث أي كسر في التطبيق، يجب الالتزام التام بعدم التعديل على الطبقات التالية:
- **إدارة الحالة (State Management):** يستخدم التطبيق `BLoC/Cubit` (مكتبة `flutter_bloc`).
- **حقن التبعيات (DI):** يتم عبر مكتبة `get_it` (تم تهيئته في `lib/core/di/injection_container.dart`).
- **طبقة البيانات (Data Layer):** ملفات `models` و `repositories` و `services` الموجودة في مجلد `lib/data/` مسؤولة عن التواصل مع `Supabase` ومعالجة البيانات، ويجب أن تبقى كما هي.
- **التواصل بين الواجهة والمنطق:** تعتمد الواجهات على استدعاء دوال الـ Cubit عبر `context.read<MyCubit>().methodName()`، ويتم جلب البيانات وعرضها باستخدام `BlocBuilder` و `BlocListener`. **حافظ على هذه الاستدعاءات كما هي في الواجهات الجديدة.**

---

## 3. تدفق البيانات والتنقل (Data Flow & Navigation)
- **نقطة الدخول:** يبدأ التطبيق من `main.dart` بتهيئة الـ Dependencies والـ Supabase. ثم يعرض `RootAuthWrapper`.
- **نظام المصادقة:** 
  - يستمع `RootAuthWrapper` لـ `AuthCubit`. 
  - إذا تم تسجيل الدخول -> يتم التوجيه لـ `LayoutScreen`.
  - إذا لم يتم التسجيل -> يتم التوجيه لـ `LoginWebScreen`.
- **الـ Layout الأساسي والصلاحيات (Roles):**
  - شاشة `LayoutScreen` هي الحاوية الأساسية للتطبيق، حالياً تستخدم `Row` مع شريط جانبي (Sidebar) على اليمين و `IndexedStack` على اليسار.
  - **مهم جداً:** يتم تحديد الشاشات التي يراها المستخدم بناءً على صلاحيته (`user.role`) من كائن `ProfileModel`. الصلاحيات المتاحة: (`sales`, `manager`, `ceo`, `admin`).
  - كل صلاحية لها مجموعة مختلفة من الشاشات التي تظهر في القائمة. **يجب الحفاظ على منطق التحقق من الصلاحيات عند تصميم شريط التنقل الجديد للهواتف.**

---

## 4. تعليمات تصميم واجهة الهاتف المحمول (Mobile UI Guidelines)
إلى النموذج القادم الذي سيصمم الواجهات، اتبع الإرشادات التالية بحذافيرها:

### أ. تهيئة المقاسات (Responsiveness)
- في `lib/main.dart`، قم بتعديل `ScreenUtilInit` من `Size(1920, 1080)` إلى مقاس مناسب للهواتف، مثلاً `Size(390, 844)` (أو اجعله ديناميكياً إن أمكن، لكن لا تعتمد على مقاسات الكمبيوتر للموبايل).
- يجب التخلص من كل ثوابت العرض المخصصة للكمبيوتر والتي تعتمد على `AppConstants.minDesktopWidth`.

### ب. الهيكل العام للتطبيق (Layout Migration)
- **استبدال Sidebar:** قم بتحويل الشريط الجانبي في `LayoutScreen` إلى **شريط تنقل سفلي (`BottomNavigationBar`)** أو **شريط تنقل (`NavigationBar` من Material 3)** للتبويبات الأساسية (مثلاً: الرئيسية، العقارات، العملاء، المهام).
- **الشاشات الإضافية:** ضع بقية الصفحات (مثل إدارة الحسابات، الأرشيف، سجل التكرارات) بداخل **`Drawer`** أو صفحة خاصة بـ "المزيد / الإعدادات".

### ج. القوائم والجداول (Lists & Data Tables)
- في نسخة الكمبيوتر الحالية يتم استخدام جداول أو `Row` مع `Expanded` لعرض بيانات العقارات والعملاء.
- في الهاتف، **قم بتحويل جميع الجداول إلى `ListView.builder`** تعرض البيانات باستخدام `Card` أو `ListTile` بشكل عمودي ومكدس (Stacked).
- لا تستخدم `SingleChildScrollView` بعرض أفقي ضخم كبديل، صمم كروتاً (Cards) واضحة تعرض المعلومات المهمة (مثل اسم العميل، رقم هاتفه، الحالة) بشكل رأسي مريح للعين.

### د. النماذج وإدخال البيانات (Forms)
- اجعل الحقول مكدسة عمودياً `Column` بدلاً من صفوف `Row`.
- استخدم `KeyboardType` الصحيح (مثل `TextInputType.phone` للأرقام).
- حافظ على استخدام وحدات التحكم `TextEditingController` الموجودة مسبقاً وتأكد من ربطها بالـ State.

### هـ. المكونات البصرية (Visual Components)
- استخدم الـ `AppTheme` الموجود حالياً في المشروع.
- قم بإزالة مكونات الفأرة (`MouseRegion`, `SystemMouseCursors.click`) واستبدلها بمكونات اللمس المناسبة للهاتف المحمول مثل `InkWell` أو `GestureDetector`.
- حافظ على الهوية البصرية (ألوان Retaj CRM الموجودة في `AppColors`).

---

## 5. خريطة الميزات والشاشات (Features Map)
ستجد جميع الواجهات في مسار `lib/features/`:
- `auth/screens/`: شاشة تسجيل الدخول.
- `dashboard/screens/`: لوحة القيادة والمؤشرات والإحصائيات.
- `properties/screens/`: قوائم العقارات وإدارة المشاركات.
- `leads/screens/`: إدارة وتتبع العملاء.
- `tasks/screens/`: إدارة المهام اليومية، بالإضافة لشاشة موافقات الإدارة.
- `profile/screens/`: الملف الشخصي وتعديل بيانات المستخدم.
- `designs/screens/`: مكتبة التصاميم والصور.
- `admin_users/screens/`: لوحة التحكم الخاصة بالمدير لإدارة الصلاحيات وحسابات المستخدمين والقوائم المنسدلة (Dropdowns).
- `archive/screens/` & `duplicates/screens/`: الأرشيف وسجلات التكرارات.

**خلاصة للنموذج القادم:** مهمتك هي التركيز بنسبة 100% على مجلد `screens` و `widgets` لجعله Mobile-First، مع بقاء مجلدات `cubit`، `services`، و `repositories` دون أي تغيير يكسر العمليات الحالية (Backend Integrations).
